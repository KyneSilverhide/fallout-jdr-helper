//TODO: Translate me !

export const MOD_HEAVY_WEAPON_DATA_IT = {
  "Fat Man": [],
  "Incinérateur Lourd": [],
  "Junk Jet": [
    {
      "Name": "Canon long",
      "Type": "Canon",
      "Effect1": "Augmente la portée d'un niveau",
      "Effect2": "",
      "Weight": "+ 1 kg",
      "Cost": "+ 20 caps",
      "Skills": "Fana d'armes 1"
    },
    {
      "Name": "Crosse à compensateur de recul",
      "Type": "Crosse",
      "Effect1": "+1 cadence de tir",
      "Effect2": "",
      "Weight": "+ 1 kg",
      "Cost": "+ 30 caps",
      "Skills": ""
    },
    {
      "Name": "Viseur d'artilleur",
      "Type": "Viseur",
      "Effect1": "Vous pouvez relancer le dé de localisation",
      "Effect2": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 5 caps",
      "Skills": ""
    },
    {
      "Name": "Module d'électrification",
      "Type": "Bouche",
      "Effect1": "Gagne Brutal",
      "Effect2": "Fait passer le type de dégâts à énergétiques",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 70 caps",
      "Skills": "Fana d'armes 2, Scientifique 1"
    },
    {
      "Name": "Module de combustion",
      "Type": "Bouche",
      "Effect1": "Gagne Persistant (énergétiques)",
      "Effect2": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 130 caps",
      "Skills": "Fana d'armes 2, Scientifique 1"
    }
  ],
  "Lance Flammes": [
    {
      "Name": "Réservoir à napalm",
      "Type": "Carburant",
      "Effect1": "+1 $CD$ de dégâts",
      "Effect2": "",
      "Weight": "+ 3,5 kg",
      "Cost": "+ 59 caps",
      "Skills": "Fana d'armes 1"
    },
    {
      "Name": "Canon long",
      "Type": "Canon",
      "Effect1": "Perd Imprécis",
      "Effect2": "",
      "Weight": "+ 1 kg",
      "Cost": "+ 28 caps",
      "Skills": "Fana d'armes 1"
    },
    {
      "Name": "Grand réservoir",
      "Type": "Réservoir à propergol",
      "Effect1": "+1 cadence de tir",
      "Effect2": "",
      "Weight": "+ 1,5 kg",
      "Cost": "+ 28 caps",
      "Skills": "Fana d'armes 1"
    },
    {
      "Name": "Réservoir géant",
      "Type": "Réservoir à propergol",
      "Effect1": "+2 cadence de tir",
      "Effect2": "",
      "Weight": "+ 3 kg",
      "Cost": "+ 34 caps",
      "Skills": "Fana d'armes 2"
    },
    {
      "Name": "Buse de compression",
      "Type": "Buse",
      "Effect1": "+1 $CD$ de dégâts",
      "Effect2": "",
      "Weight": "",
      "Cost": "+ 22 caps",
      "Skills": "Fana d'armes 1"
    },
    {
      "Name": "Buse de vaporisation",
      "Type": "Buse",
      "Effect1": "+1 $CD$ de dégâts",
      "Effect2": "Gagne Brutal",
      "Weight": "",
      "Cost": "+ 47 caps",
      "Skills": "Fana d'armes 2"
    }
  ],
  "Lance Missile": [
    {
      "Name": "Triple canon",
      "Type": "Canon",
      "Effect1": "+1 cadence de tir",
      "Effect2": "",
      "Weight": "+ 8 kg",
      "Cost": "+ 143 caps",
      "Skills": "Fana d'armes 2"
    },
    {
      "Name": "Quadruple canon",
      "Type": "Canon",
      "Effect1": "+2 cadence de tir",
      "Effect2": "",
      "Weight": "+ 10 kg",
      "Cost": "+ 218 caps",
      "Skills": "Fana d'armes 3"
    },
    {
      "Name": "Lunette",
      "Type": "Viseur",
      "Effect1": "Gagne Précis",
      "Effect2": "",
      "Weight": "+ 3 kg",
      "Cost": "+ 143 caps",
      "Skills": "Fana d'armes 2"
    },
    {
      "Name": "Lunette de vision nocturne",
      "Type": "Viseur",
      "Effect1": "Gagne Précis",
      "Effect2": "Gagne Vision nocturne",
      "Weight": "+ 3 kg",
      "Cost": "+ 248 caps",
      "Skills": "Fana d'armes 4, Scientifique 1"
    },
    {
      "Name": "Ordinateur de visée",
      "Type": "Viseur",
      "Effect1": "Quand vous visez une cible, elle ne bénéficie pas de son abri et le bonus de visée s'applique à l'attaque suivante lors de tout tour ultérieur pendant la scène",
      "Effect2": "",
      "Weight": "+ 3,5 kg",
      "Cost": "+ 293 caps",
      "Skills": "Fana d'armes 2, Scientifique 2"
    },
    {
      "Name": "Baïonnette",
      "Type": "Bouche",
      "Effect1": "Arme de corps à corps, inflige 4D de dégâts balistiques Perforant 1",
      "Effect2": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 30 caps",
      "Skills": ""
    },
    {
      "Name": "Stabilisateur",
      "Type": "Bouche",
      "Effect1": "Gagne Perforant 1",
      "Effect2": "",
      "Weight": "+ 1 kg",
      "Cost": "+ 60 caps",
      "Skills": "Fana d'armes 2"
    }
  ],
  "Gatling Laser": [
    {
      "Name": "Stimulateur de photons",
      "Type": "Condensateur",
      "Effect1": "Gagne Brutal",
      "Effect2": "",
      "Effect3": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 19 caps",
      "Skills": "Scientifique 3"
    },
    {
      "Name": "Amplificateur d'ondes Bêta",
      "Type": "Condensateur",
      "Effect1": "Gagne Persistant",
      "Effect2": "",
      "Effect3": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 57 caps",
      "Skills": ""
    },
    {
      "Name": "Condensateur amélioré",
      "Type": "Condensateur",
      "Effect1": "+1 $CD$ de dégâts",
      "Effect2": "-1 cadence de tir",
      "Effect3": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 94 caps",
      "Skills": ""
    },
    {
      "Name": "Agitateur de photons",
      "Type": "Condensateur",
      "Effect1": "+1 $CD$ de dégâts",
      "Effect2": "Gagne Brutal",
      "Effect3": "",
      "Weight": "+ 1,5 kg",
      "Cost": "+ 132 caps",
      "Skills": "Scientifique 3"
    },
    {
      "Name": "Canons à chargement",
      "Type": "Canon",
      "Effect1": "+4 $CD$ de dégâts",
      "Effect2": "-3 cadence de tir",
      "Effect3": "Augmente la portée d'un niveau",
      "Weight": "+ 5 kg",
      "Cost": "+ 357 caps",
      "Skills": "Scientifique 4"
    },
    {
      "Name": "Viseur laser",
      "Type": "Viseur",
      "Effect1": "Perd Imprécis",
      "Effect2": "",
      "Effect3": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 169 caps",
      "Skills": "Scientifique 4"
    },
    {
      "Name": "Concentrateur de faisceau",
      "Type": "Buse",
      "Effect1": "Gagne Perforant 1",
      "Effect2": "Augmente la portée d'un niveau",
      "Effect3": "",
      "Weight": "",
      "Cost": "+ 22 caps",
      "Skills": ""
    }
  ],
  "Minigun": [
    {
      "Name": "Canon grande vitesse",
      "Type": "Canon",
      "Effect1": "+1 $CD$ de dégâts",
      "Effect2": "+1 cadence de tir",
      "Effect3": "Réduit la portée d'un niveau",
      "Weight": "+ 2,5 kg",
      "Cost": "+ 45 caps",
      "Skills": "Fana d'armes 3"
    },
    {
      "Name": "Triple canon",
      "Type": "Canon",
      "Effect1": "+2 $CD$ de dégâts",
      "Effect2": "-2 cadence de tir",
      "Effect3": "",
      "Weight": "+ 1,5 kg",
      "Cost": "+ 75 caps",
      "Skills": "Fana d'armes 4"
    },
    {
      "Name": "Viseur d'artilleur",
      "Type": "Viseur",
      "Effect1": "Perd Imprécis",
      "Effect2": "",
      "Effect3": "",
      "Weight": "+ 0,5 kg",
      "Cost": "+ 68 caps",
      "Skills": ""
    },
    {
      "Name": "Broyeur",
      "Type": "Bouche",
      "Effect1": "Arme de corps à corps, inflige un nombre de $CD$ de dégâts balistiques égal à la cadence de tir",
      "Effect2": "",
      "Effect3": "",
      "Weight": "+ 2,5 kg",
      "Cost": "+ 5 caps",
      "Skills": "Fana d'armes 2"
    }
  ]
}
